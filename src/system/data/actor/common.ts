...
export interface CommonActorData {
  ...
  resources: Record<
    Resource,
    { value: number; max: number; bonus: number; deflect?: number }
  >;
...
export class CommonActorDataModel extends foundry.abstract.TypeDataModel {
        ...
        (schemas, key) => {
          schemas[key] = new foundry.data.fields.SchemaField({
            attribute: new foundry.data.fields.StringField({
              ...
              nullable: false,
              blank: false,
              initial: skills[key as unknown as Skill].attribute,
            }),
  ...
  public prepareDerivedData(): void {
    ...
    // Derive defenses
    (Object.keys(this.defenses) as unknown as AttributeGroup[]).forEach(
      (group) => {
        // Get bonus
    ...
    // Derive resource max
    (Object.keys(this.resources) as unknown as Resource[]).forEach((key) => {
      // Get the resource
    ...
    (Object.keys(this.skills) as unknown as Skill[]).forEach((skill) => {
    ...
    // Lifting & Carrying
    this.encumbrance.lift = strengthToLiftingCapacity(
      this.attributes.str.value,
    );
    this.encumbrance.carry = strengthToCarryingCapacity(
      this.attributes.str.value,
    );
  }
}
